...
namespace WebApp.AppFramework
{
    public static class DbUtils
    {
        ...
        /// <summary>
        /// </summary>
        public static List<string> GetProjectionMethodsAssignedToAuthNet(string orgName, string netName)
        {
            var results = _WebAppDbAccess.GetDataTable(string.Format(@"IF EXISTS
											(
											SELECT {0}
											  FROM ADO.Persons
											  where blah = '{1}'
											...
											SELECT {0}
											  ...
											  where blah = '{1}'
											ELSE
											SELECT 'The person does not exist';"));

            List<string> resultsToListString = DataUtils.DatatableToListString(results);
            return resultsToListString;
        }
...
where  = ''", personName);
        ...
        /// </summary>
        public static void DeleteMAPRecordSets(PersonRecord recordSet)
        {
